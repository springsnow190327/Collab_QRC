...
void ContourGraph::UpdateContourGraph(const NavNodePtr& odom_node_ptr,
    ...
    // Delegate to the layered variant with a single layer anchored at the
    // odom z — the flat path stays behaviourally identical for single-layer
    // consumers (IsMultiLayer=false).
    std::vector<std::vector<std::vector<Point3D>>> layered = {filtered_contours};
    const float anchor_z = odom_node_ptr ? odom_node_ptr->position.z : 0.0f;
    std::vector<float> layer_z = {anchor_z};
    this->UpdateContourGraph(odom_node_ptr, layered, layer_z);
}

void ContourGraph::UpdateContourGraph(const NavNodePtr& odom_node_ptr,
                                      const std::vector<std::vector<std::vector<Point3D>>>& layered_contours,
                                      const std::vector<float>& layer_z_centers) {
    odom_node_ptr_ = odom_node_ptr;
    this->ClearContourGraph();
    const float oversize_diag = FARUtil::kSensorRange * ctgraph_params_.kOversizeRangeRatio;
    const std::size_t n_layers = layered_contours.size();
    for (std::size_t li = 0; li < n_layers; li++) {
        const auto& contours_this_layer = layered_contours[li];
        const float layer_z = (li < layer_z_centers.size()) ? layer_z_centers[li] : 0.0f;
        for (const auto& poly : contours_this_layer) {
            // Drop polygons whose bounding-box diagonal exceeds oversize_diag.
            // Catches sensor-boundary artifacts that wrap the whole scan range,
            // while letting real room-scale polygons pass through.
            if (poly.size() > 2) {
                ...
                if (std::sqrt(dx * dx + dy * dy) > oversize_diag) continue;
            }
            PolygonPtr new_poly_ptr = NULL;
            this->CreatePolygon(poly, new_poly_ptr);
            if (new_poly_ptr) new_poly_ptr->layer_z = layer_z;
    ...
    if (FARUtil::IsMultiLayer) {
        this->BuildLayerTransitions();
    }
}

void ContourGraph::BuildLayerTransitions() {
    ContourGraph::layer_transitions_.clear();
    // Group contour graph ctnodes by their polygon's layer_z.
    // Within each group we keep the pillar-or-centroid representative node
    // (first ctnode of each polygon). We already accumulated that into
    // polys_ctnodes_ above; iterate it.
    if (ContourGraph::polys_ctnodes_.empty()) return;

    // Bucket representative ctnodes by layer_z (approximate equality).
    // We key by an integer layer index derived from the representative's
    // poly_ptr->layer_z, rounded to the nearest vehicle_height bucket.
    const float layer_bucket = std::max(FARUtil::kTolerZ * 2.0f, 0.25f);
    auto bucket_of = [&](float z) {
        return static_cast<int>(std::round(z / layer_bucket));
    };
    std::map<int, std::vector<CTNodePtr>> layer_buckets;
    for (const auto& ctn : ContourGraph::polys_ctnodes_) {
        if (ctn == NULL || ctn->poly_ptr == NULL) continue;
        layer_buckets[bucket_of(ctn->poly_ptr->layer_z)].push_back(ctn);
    }
    if (layer_buckets.size() < 2) return;  // only one layer observed

    // For each adjacent bucket pair (i, i+1), cross-match polygons that
    // overlap in xy. Overlap is approximated via centroid distance in xy.
    const float overlap_radius = std::max(FARUtil::kNavClearDist * 1.5f, 1.0f);

    auto xy_centroid = [](const PolygonPtr& p) {
        Point3D c(0, 0, 0);
        if (!p || p->vertices.empty()) return c;
        for (const auto& v : p->vertices) c = c + v;
        c = c / static_cast<float>(p->vertices.size());
        return c;
    };

    auto it = layer_buckets.begin();
    auto prev = it++;
    for (; it != layer_buckets.end(); ++it, ++prev) {
        if (it->first - prev->first > 1) continue;  // not strictly adjacent layers
        for (const auto& ctn_a : prev->second) {
            if (ctn_a == NULL || ctn_a->poly_ptr == NULL) continue;
            const Point3D c_a = xy_centroid(ctn_a->poly_ptr);
            for (const auto& ctn_b : it->second) {
                if (ctn_b == NULL || ctn_b->poly_ptr == NULL) continue;
                const Point3D c_b = xy_centroid(ctn_b->poly_ptr);
                const float dx = c_a.x - c_b.x;
                const float dy = c_a.y - c_b.y;
                if (std::sqrt(dx * dx + dy * dy) > overlap_radius) continue;
                ContourGraph::layer_transitions_.emplace_back(ctn_a, ctn_b);
            }
        }
    }
}
