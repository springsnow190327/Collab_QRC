...
inline double findCircumscribedCost(std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
  ...
  // ROS 1 costmap_2d uses boost::shared_ptr for layer plugins.
  std::vector<boost::shared_ptr<nav2_costmap_2d::Layer>>::iterator layer;
  ...
  for (layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    ++layer)
  {
    boost::shared_ptr<nav2_costmap_2d::InflationLayer> inflation_layer =
      boost::dynamic_pointer_cast<nav2_costmap_2d::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
