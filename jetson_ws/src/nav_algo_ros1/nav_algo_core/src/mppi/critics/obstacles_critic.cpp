...
void ObstaclesCritic::initialize()
{
  ...
  // ROS 1 Costmap2DROS has no getUseRadius() introspection. Trust the
  // user's consider_footprint param: yaml caller is responsible for matching
  // it to the costmap's footprint vs robot_radius configuration. Mismatch is
  // surfaced at planning time via collision_cost spikes instead of here.
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
  ...
  for (auto layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    ++layer)
  {
    // ROS 1 costmap_2d uses boost::shared_ptr for layer plugins (vs ROS 2's
    // std::shared_ptr). dynamic_pointer_cast must come from the matching ns.
    auto inflation_layer = boost::dynamic_pointer_cast<nav2_costmap_2d::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
