    ...
    std::vector<int>& ordered_cell_indices, const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  {
    // RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "GridWorld::SolveGlobalTSP: using nearest keypose node for robot position");
    global_path_robot_position = keypose_graph->GetNodePosition(cur_keypose_graph_node_ind_);
  }
  else
  {
    // RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "GridWorld::SolveGlobalTSP: using neighbor cell roadmap connection points for robot position");
  ...
  if (exploring_cell_indices.empty())
  {
    ...
    else
    {
      ...
      {
        // RCLCPP_ERROR(rclcpp::get_logger("tare_ros2_exact"), "Cannot find path home");
        // TODO: find a path
      }
    }
    return global_path;
