...
public:
    ...
    */
    ...
    /**
     * Multi-layer variant: extract contours for each z-band layer.
     * @param odom_node_ptr current odom node pointer
     * @param layer_clouds one PointCloudPtr per layer (already z-partitioned)
     * @param layer_z_centers representative z for each layer (for tagging)
     * @param layered_contours [return] contours[layer_idx][contour_idx] -> points
     */
    void BuildTerrainImgAndExtractContourLayered(
