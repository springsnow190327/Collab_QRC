...
void MapHandler::Init(const MapHandlerParams& params) {
    ...
    // Multi-layer snapshot grid (same geometry as terrain_height_grid_).
    terrain_multilayer_heights_ = std::make_unique<grid_ns::Grid<std::vector<float>>>(
        height_grid_size, temp_vec, height_grid_origin, height_grid_resolution, 3);
...
void MapHandler::SetTerrainHeightGridOrigin(const Point3D& robot_pos) {
    ...
    terrain_multilayer_heights_->SetOrigin(grid_origin);
}

void MapHandler::SnapshotMultilayerHeights() {
    if (!FARUtil::IsMultiLayer) return;
    // Cluster each cell's raw z-hits into layer groups.
    // A layer is a contiguous z-band of thickness <= layer_tol.
    const float layer_tol = std::max(0.4f * map_params_.floor_height, 2.0f * FARUtil::kTolerZ);
    const int n = terrain_multilayer_heights_->GetCellNumber();
    for (int i = 0; i < n; i++) {
        auto& dst = terrain_multilayer_heights_->GetCell(i);
        dst.clear();
        if (terrain_grid_occupy_list_[i] == 0) continue;
        std::vector<float> src = terrain_height_grid_->GetCell(i);  // copy
        if (src.empty()) continue;
        std::sort(src.begin(), src.end());
        float cluster_sum = src[0];
        int cluster_cnt = 1;
        float cluster_start = src[0];
        for (std::size_t k = 1; k < src.size(); k++) {
            if (src[k] - cluster_start <= layer_tol) {
                cluster_sum += src[k];
                cluster_cnt++;
            } else {
                dst.push_back(cluster_sum / static_cast<float>(cluster_cnt));
                cluster_sum = src[k];
                cluster_cnt = 1;
                cluster_start = src[k];
            }
...
float MapHandler::NearestLayerHeightForZ(const Point3D& point, const float& ref_z, bool& is_matched) {
    is_matched = false;
    if (!FARUtil::IsMultiLayer) {
        // Fall back to single-layer lookup.
        return TerrainHeightOfPoint(point, is_matched, false);
    }
    const Eigen::Vector3i sub = terrain_multilayer_heights_->Pos2Sub(Eigen::Vector3d(point.x, point.y, 0.0f));
    if (!terrain_multilayer_heights_->InRange(sub)) return ref_z;
    const int ind = terrain_multilayer_heights_->Sub2Ind(sub);
    const std::vector<float>& heights = terrain_multilayer_heights_->GetCell(ind);
    if (heights.empty()) return ref_z;
    float best_h = heights.front();
    float best_d = std::abs(best_h - ref_z);
    for (std::size_t k = 1; k < heights.size(); k++) {
        const float d = std::abs(heights[k] - ref_z);
        if (d < best_d) {
            best_d = d;
            best_h = heights[k];
        }
...
std::vector<float> MapHandler::GetLayerHeights(const Point3D& point) {
    std::vector<float> out;
    if (!FARUtil::IsMultiLayer) return out;
    const Eigen::Vector3i sub = terrain_multilayer_heights_->Pos2Sub(Eigen::Vector3d(point.x, point.y, 0.0f));
    if (!terrain_multilayer_heights_->InRange(sub)) return out;
    const int ind = terrain_multilayer_heights_->Sub2Ind(sub);
    return terrain_multilayer_heights_->GetCell(ind);  // copy
}
...
void MapHandler::GetSurroundObsCloudLayers(std::vector<PointCloudPtr>& obsLayersOut,
                                           std::vector<float>& layerCentersOut) {
    obsLayersOut.clear();
    layerCentersOut.clear();
    if (!is_init_) return;

    // Build the full surround cloud first.
    PointCloudPtr full_ptr(new pcl::PointCloud<PCLPoint>());
    this->GetSurroundObsCloud(full_ptr);

    if (!FARUtil::IsMultiLayer) {
        obsLayersOut.push_back(full_ptr);
        layerCentersOut.push_back(FARUtil::robot_pos.z);
        return;
    }

    // Discover layer z-values by scanning terrain_multilayer_heights_ in a
    // local window around the robot. Cluster them across cells with the
    // same layer tolerance used during SnapshotMultilayerHeights.
    const float layer_tol = std::max(0.4f * map_params_.floor_height, 2.0f * FARUtil::kTolerZ);
    std::vector<float> raw_heights;
    const Eigen::Vector3i robot_sub = terrain_multilayer_heights_->Pos2Sub(
        Eigen::Vector3d(FARUtil::robot_pos.x, FARUtil::robot_pos.y, 0.0f));
    const Eigen::Vector3i dims = terrain_multilayer_heights_->GetSize();
    const int half = static_cast<int>(std::ceil(map_params_.sensor_range / FARUtil::robot_dim));
    for (int dx = -half; dx <= half; dx++) {
        for (int dy = -half; dy <= half; dy++) {
            Eigen::Vector3i sub = robot_sub;
            sub.x() += dx;
            sub.y() += dy;
            if (sub.x() < 0 || sub.y() < 0 || sub.x() >= dims.x() || sub.y() >= dims.y()) continue;
            const int ind = terrain_multilayer_heights_->Sub2Ind(sub);
            const auto& hs = terrain_multilayer_heights_->GetCell(ind);
            raw_heights.insert(raw_heights.end(), hs.begin(), hs.end());
        }
    }

    // Cluster the collected layer heights.
    std::vector<float> layer_centers;
    if (!raw_heights.empty()) {
        std::sort(raw_heights.begin(), raw_heights.end());
        float cluster_sum = raw_heights[0];
        int cluster_cnt = 1;
        float cluster_start = raw_heights[0];
        for (std::size_t k = 1; k < raw_heights.size(); k++) {
            if (raw_heights[k] - cluster_start <= layer_tol) {
                cluster_sum += raw_heights[k];
                cluster_cnt++;
            } else {
                layer_centers.push_back(cluster_sum / static_cast<float>(cluster_cnt));
                cluster_sum = raw_heights[k];
                cluster_cnt = 1;
                cluster_start = raw_heights[k];
            }
        }
        layer_centers.push_back(cluster_sum / static_cast<float>(cluster_cnt));
    }

    if (layer_centers.empty()) {
        // Fallback: one layer at robot z.
        obsLayersOut.push_back(full_ptr);
        layerCentersOut.push_back(FARUtil::robot_pos.z);
        return;
    }

    const std::size_t n_layers = layer_centers.size();
    obsLayersOut.resize(n_layers);
    for (std::size_t li = 0; li < n_layers; li++) {
        obsLayersOut[li] = PointCloudPtr(new pcl::PointCloud<PCLPoint>());
    }
    layerCentersOut = layer_centers;

    // Bucket each obs point into its nearest layer (by z).
    for (const auto& p : full_ptr->points) {
        std::size_t best = 0;
        float best_d = std::abs(p.z - layer_centers[0]);
        for (std::size_t li = 1; li < n_layers; li++) {
            const float d = std::abs(p.z - layer_centers[li]);
            if (d < best_d) {
                best_d = d;
                best = li;
            }
        }
        // Only keep points within half layer spacing of a layer centre.
        // This discards points that sit above ceilings / below floors.
        if (best_d <= map_params_.floor_height) {
            obsLayersOut[best]->points.push_back(p);
        }
    }
    for (auto& cloud : obsLayersOut) {
...
void MapHandler::UpdateTerrainHeightGrid(const PointCloudPtr& freeCloudIn, const PointCloudPtr& terrainHeightOut) {
    ...
    // Multi-layer: snapshot raw per-cell z-hits into the parallel grid BEFORE
    // TraversableAnalysis collapses terrain_height_grid_ to single values.
    this->SnapshotMultilayerHeights();
