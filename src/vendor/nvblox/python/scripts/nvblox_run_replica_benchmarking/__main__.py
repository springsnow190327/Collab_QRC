...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--do_display_error_histogram',
    ...
    parser.add_argument(
        '--kpi_namespace',
        type=str,
        help='If passed, the KPIs in the output JSON will be preceeded by this string.')
...
def get_output_file_path(output_root_path: Path, file_name: str) -> Path:
...
def get_groundtruth_mesh_path(dataset_path: Path) -> Path:
    """Check if the ground truth mesh file exists and return its path."""
    sequence_name = dataset_path.stem
    groundtruth_mesh_path = Path(dataset_path) / '..' / f'{sequence_name}_mesh.ply'

    if not groundtruth_mesh_path.is_file():
        raise FileNotFoundError(f'Groundtruth mesh does not exist:{groundtruth_mesh_path}')

    return groundtruth_mesh_path


def write_merged_kpis(dataset_path: Path,
                      output_root_path: Path,
                      kpi_namespace: Optional[str] = None) -> None:
    """Write a dict with merged kpis."""
    merged_kpis = {}
    sequence_name = dataset_path.stem
    for kpi_file in ['esdf_error_statistics.json', 'surface_error_statistics.json', 'timing.json']:
        with open(Path(output_root_path) / sequence_name / kpi_file, 'r', encoding='utf-8') as fp:
            merged_kpis.update(json.load(fp))

    # Push KPIs into a namespace if requested
    if kpi_namespace:
        kpi_namespace = kpi_namespace.rstrip('/')
        merged_kpis = {(kpi_namespace + '/' + key): val for key, val in merged_kpis.items()}
...
def main(args: argparse.Namespace) -> None:
    ...
    reconstructed_mesh_path = get_output_file_path(args.output_root_path, 'reconstructed_mesh.ply')
    ...
    reconstructed_esdf_path = get_output_file_path(args.output_root_path, 'reconstructed_esdf.ply')
    ...
    write_merged_kpis(args.dataset_path, args.output_root_path, args.kpi_namespace)
