...
#include "nav2_mppi_controller/cuda_backend.hpp"  // pure-virtual ICudaBackend interface
...
public:
  ...
  // Optional GPU acceleration hook. When set (typically by the CudaMPPI
  // controller plugin reading a `use_cuda: true` yaml param), Optimizer::
  // optimize() delegates the per-iteration body to the backend. When null
  // (default), the xtensor CPU path runs unchanged. NOTE: this hook only
  // affects this vendored copy; apt-installed nav2_mppi_controller is
  // untouched.
  void setCudaBackend(mppi::ICudaBackend * backend) { cuda_backend_ = backend; }
  ...
  // Signatures kept byte-equivalent to the ROS 1 nav_algo_core fork so a
  // single CudaBackend translation unit serves both builds.
  ...
  bool isHolonomicPublic() const
  {return motion_model_ ? motion_model_->isHolonomic() : false;}
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> getCostmapRosForBackend()
  {return costmap_ros_;}
