...
void RollingOccupancyGrid::RayTrace(const Eigen::Vector3d& origin, const Eigen::Vector3d& range)
{
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "RollingOccupancyGrid::RayTrace(), robot not in range");
    return;
  }
                                       ...
                                       const Eigen::Vector3d& origin, const Eigen::Vector3d& range)
{
  ...
  if (!occupancy_array_->InRange(origin_sub))
  {
    RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "RollingOccupancyGrid::GetFrontierInRange(), robot not in range");
    return;
  }
  int ray_trace_count = 0;
