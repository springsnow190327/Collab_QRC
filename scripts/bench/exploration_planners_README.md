...
Common-executor benchmark for `cfpa2`, `gbplanner3`, and `mtare` in the
...
- GBPlanner3: `/robot_a/command/trajectory`, `/robot_b/command/trajectory`
- MTARE/TARE: `/robot_a/mtare/way_point`, `/robot_b/mtare/way_point`

Adapters relay those outputs to `/<robot_ns>/way_point_coord`.

GBPlanner3 uses `scripts/sim/gbplanner3_mujoco/launch_dual_common_executor.sh`
by default.  It requires `UAS_REPO_ROOT` to point at the Unified Autonomy Stack
checkout with the GBPlanner3 Docker images built.
