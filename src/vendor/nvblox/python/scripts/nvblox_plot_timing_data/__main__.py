...
START_TOKENS = {
    'timings': 'namespace/tag - NumSamples - TotalTime - (Mean +- StdDev) - [Min,Max]',
    'rates': 'namespace/tag - NumSamples (Window Length) - Mean',
    'delays': 'namespace/tag - NumSamples (Window Length) - Mean Delay (seconds)'
}
...
def filter_data(raw_data: str, include_regexp: str, max_namespace_level: int) -> str:
...
def extract_data(logfile: str, start_token: str, include_regexp: str,
                 max_namespace_level: int) -> pd.DataFrame:
    ...
    filtered_data = filter_data(matches[-1], include_regexp, max_namespace_level)
    print('Data block from log:')
    print(filtered_data)

    # Create a dataframe out of the  filtered data
    rows = [row.strip() for row in filtered_data.strip().split('\n') if row.strip()]
...
def create_plot(data_frames: list, column: int, label: str, data_names: list) -> Figure:
    ...
    for name, df in zip(data_names, data_frames):
        try:
            ...
            print(df)
            print(f'ERROR: Invalid column: {column}. Select one from the table above')
            sys.exit(1)

    fig = px.bar(data_dict, x=data_names, barmode='group', labels=data_names, log_x=False)
    fig.update_layout(xaxis_title=label, yaxis_title='', title='nvblox performance measurement')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--mode',
                        ...
                        required=True)
    parser.add_argument('--label', '-l', type=str, help='What to plot', required=True)
    ...
    parser.add_argument('--output-json',
                        ...
                        default=None)
    parser.add_argument('--max-namespace-level',
                        '-n',
                        type=int,
...
def main(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print('Use one of: [' + str(', '.join(COLUMN_INDICES[args.mode].keys())) + ']')
        sys.exit(1)

    dfs = []
    for logfile in args.input_logfiles:
        df = extract_data(logfile, start_token, args.regexp, args.max_namespace_level)
        if df is None:
            sys.exit(1)
        dfs.append(df)
