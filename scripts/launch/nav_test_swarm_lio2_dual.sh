#!/usr/bin/env bash
# nav_test_swarm_lio2_dual.sh — sim 2× Go2W + CHAMP + Swarm-LIO2 (ROS1 in
# docker, drone_id=1/2) + FAR + CFPA2 dual-robot frontier coordination.
#
# Cousin of nav_test_swarm_lio2_se2.sh (single Go2) and
# nav_test_mujoco_fastlio_dual.sh — Fast-LIO2 replaced by the dockerized
# ROS1 Swarm-LIO2 stack on both robots. The launch starts the docker
# compose stack once (bridge.yaml already exposes /robot_{a,b}/swarm_lio2_raw/*)
...
#
# Usage:
#   ./scripts/launch/nav_test_swarm_lio2_dual.sh                 # gui:=false (default)
#   ./scripts/launch/nav_test_swarm_lio2_dual.sh gui:=true rviz:=true
#   ./scripts/launch/nav_test_swarm_lio2_dual.sh explore:=false  # manual goals only
set -euo pipefail
...
echo "=== MuJoCo Dual Nav Test (Swarm-LIO2 hybrid + FAR + CFPA2) ==="
echo ""

exec ros2 launch go2_gazebo_sim nav_test_mujoco_swarm_lio2_dual.launch.py "$@"
