...
struct ContourGraphParams {
    ...
    float kOversizeRangeRatio = 3.0f;  // drop polygons with diag > ratio * sensor_range
};
...
public:
    ...
    /**
     * Multi-layer variant: build the contour graph from contours already
     * partitioned per layer. Polygons are tagged with their layer z-value.
     * For single-layer callers, the non-layered overload above delegates to
     * this one with a one-element layer vector.
     */
    void UpdateContourGraph(const NavNodePtr& odom_node_ptr,
    ...
    /**
     * Multi-layer: read-only accessor for the current per-tick list of layer
     * transition edges. Each element is a (CTNode on layer A, CTNode on layer
     * B) pair where polygons on adjacent layers overlap in xy. Returns an
     * empty vector when IsMultiLayer is false or no transitions were found.
     */
    static const std::vector<std::pair<CTNodePtr, CTNodePtr>>& GetLayerTransitions() {
        return layer_transitions_;
    }
...
private:
    ...
    static std::vector<std::pair<CTNodePtr, CTNodePtr>> layer_transitions_;
    ContourGraphParams ctgraph_params_;

    /**
     * Multi-layer: after polygons have been built per layer, scan adjacent
     * layers for xy-overlapping polygons and add a transition edge between
     * their representative CTNodes. Called from UpdateContourGraph only
     * when IsMultiLayer is true.
     */
    void BuildLayerTransitions();
