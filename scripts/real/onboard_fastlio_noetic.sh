#!/usr/bin/env bash
# onboard_fastlio_noetic.sh — bring up Mid-360 + FAST-LIO2 in ROS 1 Noetic on
...
#   ./onboard_fastlio_noetic.sh                          # default ns=robot
#   ./onboard_fastlio_noetic.sh namespace=robot_b
#   ./onboard_fastlio_noetic.sh rviz=true                # local visualisation
#   ./onboard_fastlio_noetic.sh stop                     # tear down everything
#
...
# (fastlio_mapping, livox_ros_driver2_node) then segfault before main()
...
# ── Cleanup ──────────────────────────────────────────────────────────
_kill_noetic_stack() {
  for p in fastlio_mapping laserMapping livox_ros_driver2_node \
...
if ! rospack find fast_lio &>/dev/null; then
  echo "ERROR: rospack can't find 'fast_lio'. catkin build incomplete?" >&2
  exit 1
fi
...
# ROS networking: Jetson's ROS_IP must be reachable BY the master so return
# connections (subscriptions, service calls) work.  On pure Ethernet this is
# always 192.168.123.18 (or whatever the Jetson's eth IP is).
...
if [[ -n "$EXTERNAL_MASTER" ]]; then
  # Laptop is the ROS master — skip roscore, point nodes at laptop.
...
echo ""
echo "################################################"
echo "  Noetic FAST-LIO2 (Jetson @ $(hostname))"
...
echo "  Stop          : Ctrl+C  or  ./onboard_fastlio_noetic.sh stop
...
if [[ -n "$EXTERNAL_MASTER" ]]; then
  ...
  if ! rostopic list &>/dev/null; then
    ...
    echo "         source /opt/ros/noetic/setup.bash && roscore" >&2
    cleanup_on_signal
  fi
else
...
echo "[3/5] Starting FAST-LIO2 (mapping_mid360, ns=${NAMESPACE})..."
ROS_NAMESPACE="$NAMESPACE" nohup \
  roslaunch fast_lio mapping_mid360.launch \
    rviz:=false \
    </dev/null >/tmp/fastlio_noetic.log 2>&1 &
FASTLIO_LAUNCH_PID=$!
disown $FASTLIO_LAUNCH_PID 2>/dev/null || true
echo "      PID=$FASTLIO_LAUNCH_PID  log: /tmp/fastlio_noetic.log"
...
if [[ "$ENABLE_RVIZ" == "true" ]]; then
  ...
  RVIZ_CFG="$(rospack find fast_lio)/rviz_cfg/loam_livox.rviz"
...
echo ""
echo "  Stop:   ${WS_ROOT}/scripts/onboard_fastlio_noetic.sh stop"
