...
def evaluate_mesh(reconstructed_mesh_path: Path,
    ...
    """
    Evaluate a reconstructed mesh.

    Calculate error between a reconstruction and the groundtruth geometry.

    Args:
    ----
    reconstructed_mesh_path (Path): Path to the reconstructed surface to be evaluated.
    ...
    do_error_visualization (bool, optional): Whether or not to display the error plots.
    ...
    statistics_dict = {
        'surface_error_mean': np.mean(per_vertex_errors),
