...
{
  if (!loader_) {
    // Vendored copy: critic XML is exported under the nav2_mppi_controller_cuda
    // package name so this lookup doesn't collide with apt-installed
    // ros-humble-nav2-mppi-controller's plugin descriptor.
    loader_ = std::make_unique<pluginlib::ClassLoader<critics::CriticFunction>>(
      "nav2_mppi_controller_cuda", "mppi::critics::CriticFunction");
  }
