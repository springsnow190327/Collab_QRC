...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh("~");
    // Patch: separate public handle for publishers. Upstream uses nh("~")
    // for BOTH params and topic advertise → topics end up under the node's
    // private namespace (/<ns>/laserMapping/Odometry). We want /<ns>/Odometry
    // for drop-in compat with FAST-LIO2 downstream consumers (gbplanner3,
    // record/replay/stream scripts).
    ...
    // Patch: topics relative + use PUBLIC handle (pub_nh). ROS_NAMESPACE=robot
    // prefixes them automatically to /robot/<topic>. Matches the FAST-LIO2
    // drop-in interface in src/vendor/fast_lio_ros1/, keeping our downstream
    // record/replay/stream scripts source-compatible. Odometry renamed from
    // upstream "aft_mapped_to_init" to "Odometry" for the same reason.
    ros::Publisher pubLaserCloudFullRes = pub_nh.advertise<sensor_msgs::PointCloud2>
            ("cloud_registered", 1000);
    ros::Publisher pubLaserCloudFullRes_body = pub_nh.advertise<sensor_msgs::PointCloud2>
            ("cloud_registered_body", 1000);
    // ros::Publisher pubLaserCloudEffect  = pub_nh.advertise<sensor_msgs::PointCloud2>
            // ("cloud_effected", 1000);
    ros::Publisher pubLaserCloudMap = pub_nh.advertise<sensor_msgs::PointCloud2>
            ("Laser_map", 1000);
    ros::Publisher pubOdomAftMapped = pub_nh.advertise<nav_msgs::Odometry>
            ("Odometry", 1000);
    ros::Publisher pubPath          = pub_nh.advertise<nav_msgs::Path>
            ("path", 1000);
