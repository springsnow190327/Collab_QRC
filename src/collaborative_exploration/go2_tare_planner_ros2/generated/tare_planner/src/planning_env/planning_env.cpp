...
void PlanningEnv::UpdateTerrainCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud)
{
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "Terrain cloud empty");
  }
  else
  {
...
bool PlanningEnv::InCollision(double x, double y, double z) const
{
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("tare_ros2_exact"), "PlanningEnv::InCollision(): collision cloud empty, not checking collision");
    return false;
  }
