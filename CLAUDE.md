...
- **Onboard SLAM split — Fast-LIO + Livox driver running on the Go2 Jetson over cross-host DDS (2026-04-30)** — half-day debugging journey from "deploy SLAM onboard" to "every ROS 2 node uses 13 GB" to "Foxy install is broken" to "actually it's a one-line `ulimit -v` fix". Per-bug history below; consolidated runbook at [docs/claude/real_robot.md](docs/claude/real_robot.md#onboard-slam-split-shipped-2026-04-30--fast-lio--livox-on-jetson).
  1. **Goal & motivation** — laptop is doing too much (Fast-LIO + Nav2 + CFPA2 + octomap + RViz×2 + safety stack on a single 8-core), MPPI plans take 3 min on real, controller drops trajectories under load. Push SLAM onto the Jetson at `192.168.123.18` (Tegra Orin, Ubuntu 20.04, ROS 2 Foxy). Future Jetson Orin Super gets perception/GPU work; this Jetson keeps SLAM.
  2. **Architectural risk: cross-distro DDS** — laptop runs Humble + CycloneDDS, Jetson runs Foxy + (eventually) FastDDS. DDS RTPS wire format is interoperable across implementations and ROS 2 distros, but the *configuration* schemas differ. Verified empirically that Cyclone-Humble ↔ FastDDS-Foxy talk fine over Ethernet once both sides are on `ROS_DOMAIN_ID=0` and peer each other.
  3. **Five Foxy ↔ Humble source patches** — vendored packages target Humble; building on Foxy needed:
     - `livox_ros_driver2/src/comm/pub_handler.cpp:135` references `kLivoxLidarTypeMid360s` (newer SDK enum). Patched out — we have plain Mid-360, no `s` variant.
     - `fast_lio/src/laserMapping.cpp:1124` uses `std_srvs::srv::Trigger::Request::ConstSharedPtr` — Humble-only alias. Foxy expects `Request::SharedPtr` (drop the `Const`). Without this fix, build error: `enable_if<false, void>::type does not exist` in rclcpp's `AnyServiceCallback::set` overload resolution.
     - CycloneDDS XML schema differs: Foxy 0.7.x uses `<NetworkInterfaceAddress>...</>`, Humble 0.10+ uses `<Interfaces><NetworkInterface .../></Interfaces>`. Mismatched XML → `unknown element` → `rmw_handle invalid` → every node crashes with `rcl node's rmw handle is invalid`. Branched in [`scripts/real/onboard_slam.sh`](scripts/real/onboard_slam.sh).
     - `static_transform_publisher` argument syntax: Humble takes `--frame-id parent --child-frame-id child --x 0 ...`. Foxy takes positional `x y z qx qy qz qw parent child` (or `yaw pitch roll` Euler form, **note order — yaw first, not roll**). Wrong syntax → `static_transform_publisher exited due to not having the right number of arguments`.
     - Foxy parameter parser rejects empty value `-p lvx_file_path:=` with `Couldn't parse parameter override rule`. Workaround: `-p lvx_file_path:='""'` (quoted empty string).
  4. **Foxy rclpy 0.9.x segfaults under any non-trivial Python ROS 2 node** — `fast_lio_tf_adapter.py` (the laptop's adapter) and even a 30-line minimal Python relay both `Segmentation fault (core dumped)` immediately on Foxy. Demo C++ nodes work, just rclpy is broken. Workaround: have `fastlio_mapping` itself remap `/Odometry` → `/<ns>/odom/nav` via `-r` arg at launch time, no Python relay needed. SC-PGO bootstrap-from-GT logic deferred until Humble migration (Orin Super).
  5. **Detachment quirks** — `nohup bash script.sh &` over SSH leaves children tied to the controlling terminal; bash's `wait` returns when stdin EOFs and SIGHUPs propagate. Plain `&` is not enough. Each node must be launched via `setsid -f <cmd> </dev/null >log 2>&1` for full session detachment. The script became a "fire-and-forget spawner" with the wait at the end removed — children outlive the script invocation.
  6. **The 13 GB-per-node bloat — the real story** — observed: any ROS 2 C++ binary on this Tegra Foxy uses **13 GB resident, ~15 GB virtual**, even a trivial `static_transform_publisher`. 5 SLAM nodes need 65 GB on a 15 GB machine → instant OOM kill (`Killed process … (fastlio_mapping) total-vm:31038216kB, anon-rss:14950652kB`). Methodically ruled out:
     - **CUDA / Tegra UMA**: `ldd` shows zero CUDA libs linked. `CUDA_VISIBLE_DEVICES=""` no effect.
     - **glibc malloc arenas**: `MALLOC_ARENA_MAX=2`, `MALLOC_MMAP_THRESHOLD_=131072`, `MALLOC_TOP_PAD_=131072` — no effect.
     - **jemalloc / tcmalloc preload** (offline-installed via `.deb` from `ports.ubuntu.com/ubuntu-ports`): no effect. With jemalloc preloaded, virtual went to 134 GB but resident stayed 12.5 GB.
     - **Sanitizers**: `nm -D` and `readelf -d` show no `__asan` / `__tsan` / `__ubsan` symbols.
     - **DDS shared memory transports**: forced FastDDS UDP-only via custom XML profile (`<useBuiltinTransports>false</useBuiltinTransports>` + `<transport_descriptor type=UDPv4>`), bounded `max_samples=4`, `historyMemoryPolicy=DYNAMIC_RESERVE` — no effect.
     - **CycloneDDS**: process **segfaults** in `spin()` instead of bloating. Worse, not better.
     - **Thread count**: `/proc/PID/status` showed 8 threads (matches `nproc=8`), each "owning" ~1.6 GB. `taskset -c 0` reduces CPU affinity but **does not reduce thread count** — threads are spawned regardless.
     - **strace for huge mmap**: `strace` not installed on the Jetson, no internet to apt-install. Inferred from `/proc/PID/maps` instead — saw a single anonymous mapping of **14.5 GB** (one `mmap()` call), perms=`rw-p`, no file backing.
  7. **`ulimit -v 1500000` — the one-line fix** — confronted with the dead-end "every ROS 2 process eats 13 GB", a desperate attempt at a virtual-memory cap turned out to be the entire fix. Foxy/aarch64 + FastDDS speculatively `mmap`s ~14.5 GB at startup (likely an internal buffer pool sized off `/proc/meminfo`), the kernel happily grants the virtual address space, then lazy-faults committed pages as 8 threads touch them. With `ulimit -v 1500000` (1.5 GB virtual cap), the speculative `mmap` *fails*; the library catches `std::bad_alloc` (the harmless `bad_alloc caught: std::bad_alloc` log spam visible all session) and falls back to bounded buffers. Same observable behavior, **500× less memory**.
     - Before: 1 static_transform_publisher = 13 GB resident, 5-node stack = OOM in 8s.
     - After: 1 static_transform_publisher = **22 MB resident**, 5-node stack = **333 MB total**, 14 GB free.
     - Verified `/livox/{lidar,imu}` discovered cross-host from laptop, `/robot/odom/nav` publishing at **26.78 Hz** cross-host, TF `map → base_link` resolvable from laptop side.
  8. **Net result + design tax** — onboard SLAM works on this Tegra Foxy with the fixes above. The Foxy compatibility tax is real (5 source patches + 4 Foxy/Humble schema differences + a virtual-memory cap + Python rclpy segfault avoidance) but all of it is **forward-compatible to Humble** — the patches stay, the workarounds become unnecessary. When the Orin Super arrives with Ubuntu 22.04 + native Humble, the same `deploy_to_jetson.sh` re-runs cleanly, all the source patches still apply, and `ulimit -v` becomes a no-op (Humble doesn't bloat). All work is **distro-portable**.
  9. **Honest lesson on the "give up too early" near-miss** — at the 13-GB-per-node observation I had the right diagnosis ("this Foxy install is fundamentally broken") but the wrong conclusion ("not solvable in this session"). User pushed back ("what the fuck? no way?"); I went one more round, found the single anonymous 14.5 GB mmap, then tried `ulimit -v` more out of desperation than expectation — and that was the answer. **Lesson:** before declaring a hardware/runtime issue unfixable, exhaust the hard-resource-cap workarounds (`ulimit`, cgroups, address-space restrictions). Speculative allocators usually have a graceful-fallback path under cap-pressure that's invisible without forcing them into it.

  Phase 1 (laptop wiring + onboard build) is shipped; Phase 2 (SC-PGO ROS 1→2 port for loop closure) deferred to a focused next session — `src/vendor/sc_pgo/PORT_TO_ROS2.md` has the full spec, ~640 LOC of mechanical surgery + GTSAM dep + topic-name contract. Migration to Jetson Orin Super remains the long-term clean path.
...
| **Onboard SLAM split (2026-04-30): Fast-LIO + Livox onto Go2 Jetson Foxy. 5 source patches, the 13-GB-per-node bloat, the `ulimit -v` fix.** | This file's "Active state (2026-04-30)" entry + [docs/claude/real_robot.md](docs/claude/real_robot.md#onboard-slam-split-shipped-2026-04-30--fast-lio--livox-on-jetson) |
