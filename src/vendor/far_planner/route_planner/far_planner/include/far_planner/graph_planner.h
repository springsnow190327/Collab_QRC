...
struct GraphPlannerParams {
    ...
    int   votes_size;
    int   momentum_thred;
    // Multi-layer: multiplier applied to euclidean edge cost when the edge
    // crosses into a different layer (the two nav nodes report different
    // layer_z beyond kTolerZ). Default 2.0f. Has no effect when
    // FARUtil::IsMultiLayer is false.
    float layer_transition_cost_mult = 2.0f;
};
