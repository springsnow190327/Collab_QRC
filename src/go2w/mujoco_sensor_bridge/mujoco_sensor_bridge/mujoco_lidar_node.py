...
mount position using mj_ray(). Publishes sensor_msgs/PointCloud2 at 10 Hz on
/{ns}/registered_scan.
...
class MujocoLidarNode(Node):
    ...
    def __init__(self):
        ...
        # Body id still needed for mj_ray bodyexclude
        ...
        self.get_logger().info(
            ...
            f'{rate} Hz, range [{self.range_min}, {self.range_max}]m')
        ...
        self.get_logger().info('MuJoCo LiDAR node started')
    ...
    def _timer_cb(self):
        ...
        # 5. Cast all rays (returns world-frame displacement vectors)
        points_world = self._cast_rays(lidar_origin, ray_dirs_world)
        ...
        #    timestamps dispatched during the expensive mj_ray() loop.
    ...
    # ------------------------------------------------------------------
    def _cast_rays(self, origin: np.ndarray, directions: np.ndarray) -> np.ndarray:
        """Cast rays and return hit points in lidar frame (relative to origin)."""
        # Pre-allocate output
        hit_points = np.empty((self.n_rays, 3), dtype=np.float32)
        valid = np.zeros(self.n_rays, dtype=bool)
        ...
        with self._mj_lock:
            for i in range(self.n_rays):
                dist = mujoco.mj_ray(
                    ...
                    origin,
                    directions[i],
                    self._geomgroup,  # None = all groups
                    1,                # flg_static: include static geoms
                    self.lidar_body_id,  # bodyexclude: exclude robot body
                    geomid_out,
                )
                if dist >= self.range_min and dist <= self.range_max:
                    # Point in world frame relative to origin
                    hit_points[i] = (directions[i] * dist).astype(np.float32)
                    valid[i] = True
