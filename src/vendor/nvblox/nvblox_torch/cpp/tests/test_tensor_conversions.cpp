...
TEST(ConvertTensors, ViewConversion) {
  torch::Tensor tensor = torch::tensor({{{0.F}, {1.F}}, {{2.F}, {3.F}}},
                                       torch::device(torch::kCUDA));

  auto view = pynvblox::view_from_tensor<float>(tensor);

  std::vector<float> vec(4);
  cudaMemcpyAsync(vec.data(), view.dataConstPtr(), sizeof(float) * vec.size(),
                  cudaMemcpyDeviceToHost, nvblox::CudaStreamOwning());
