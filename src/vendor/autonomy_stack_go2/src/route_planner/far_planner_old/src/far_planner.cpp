...
#include "far_planner_old/far_planner.h"
...
void FARMaster::TerrainCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
  ...
  if (!is_stop_update_) {
    ...
    // ── DIAG STAGE 1: terrain intensity split ──────────────────────────────────
    {
      static int _diag_ctr1 = 0;
      if (++_diag_ctr1 % 10 == 1) {
        // Compute intensity stats on the raw cropped cloud
        float iMin = 1e9f, iMax = -1e9f, iSum = 0.f;
        for (const auto& p : temp_cloud_ptr_->points) {
          if (p.intensity < iMin) iMin = p.intensity;
          if (p.intensity > iMax) iMax = p.intensity;
          iSum += p.intensity;
        }
        const float iMean = temp_cloud_ptr_->empty() ? 0.f : iSum / temp_cloud_ptr_->size();
        RCLCPP_WARN(nh_->get_logger(),
          "[DIAG1] terrain_cloud in=%zu  free=%zu  obs=%zu  kFreeZ=%.3f  "
          "intensity[min=%.3f mean=%.3f max=%.3f]",
          temp_cloud_ptr_->size(), temp_free_ptr_->size(), temp_obs_ptr_->size(),
          FARUtil::kFreeZ, iMin, iMean, iMax);
      }
    }
    // ─────────────────────────────────────────────────────────────────────────
    if (!master_params_.is_static_env) {
  ...
  // ── DIAG STAGE 2: surround_obs_cloud_ after grid accumulation ────────────
  {
    static int _diag_ctr2 = 0;
    if (++_diag_ctr2 % 10 == 1) {
      RCLCPP_WARN(nh_->get_logger(),
        "[DIAG2] surround_obs=%zu  surround_free=%zu  is_cloud_init=%d",
        FARUtil::surround_obs_cloud_->size(),
        FARUtil::surround_free_cloud_->size(),
        (int)is_cloud_init_);
    }
  }
  // ─────────────────────────────────────────────────────────────────────────
