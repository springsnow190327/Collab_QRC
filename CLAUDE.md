...
- **Trav/inflation over-wide:** operator confirmed free space over-painted lethal (robot is NOT hitting walls — contacts are foot-floor only). Cause is Nav2 inflation (→ 0.16); if the trav-grid lethal itself is too thick on the real robot, tune `grid_map_to_occupancy_grid` thresholds / elevation wall-cost width (upstream, separate from inflation).
