...
void GraphPlanner::UpdateGraphTraverability(const NavNodePtr& odom_node_ptr, const NavNodePtr& goal_ptr) 
{
    ...
    while (!open_set.empty()) {
        ...
        for (const auto& neighbor : current->connect_nodes) {
            ...
            // Phase B.4: apply layer-transition cost multiplier when the
            // two nav nodes sit on different layers and we're in multi-layer
            // mode. This penalises stair/ramp crossings vs. flat traversal.
            if (FARUtil::IsMultiLayer && !FARUtil::IsAtSameLayer(neighbor, current)) {
                edist *= gp_params_.layer_transition_cost_mult;
            }
