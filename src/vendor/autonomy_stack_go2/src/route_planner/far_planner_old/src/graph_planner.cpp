...
#include "far_planner_old/graph_planner.h"
...
void GraphPlanner::UpdateGoalNavNodeConnects(const NavNodePtr& goal_ptr)
{
    ...
    int total = 0, traversable = 0, valid_connect = 0, edges_added = 0;
    for (const auto& node_ptr : current_graph_) {
        if (node_ptr == goal_ptr) continue;
        total++;
        if (node_ptr->is_traversable) traversable++;
    ...
    RCLCPP_WARN(nh_->get_logger(), "GoalConnect: total=%d traversable=%d valid_connect=%d edges_added=%d goal=(%.1f,%.1f,%.1f)",
        total, traversable, valid_connect, edges_added,
        goal_ptr->position.x, goal_ptr->position.y, goal_ptr->position.z);
}

bool GraphPlanner::IsValidConnectToGoal(const NavNodePtr& node_ptr, const NavNodePtr& goal_node_ptr) {
    // DEBUG: track which condition fails (only for first traversable node per cycle)
    ...
    if (node_ptr->is_traversable && (!node_ptr->is_block_to_goal || IsResetBlockStatus(node_ptr, goal_node_ptr))) {
        ...
        bool c1 = DynamicGraph::IsConvexConnect(node_ptr, goal_node_ptr);
        bool c2_same = FARUtil::IsAtSameLayer(node_ptr, goal_node_ptr);
        bool c2_outdir = FARUtil::IsOutReducedDirs(diff_p, node_ptr);
        bool c2 = !c2_same || c2_outdir;
        bool c3 = ContourGraph::IsNavToGoalConnectFreePolygon(node_ptr, goal_node_ptr);
        if (!logged && node_ptr->is_odom) {
            float dist = diff_p.norm();
            RCLCPP_WARN(nh_->get_logger(), "  ODOM->goal: convex=%d sameLayer=%d outDir=%d freePoly=%d dist=%.1f nodeZ=%.2f goalZ=%.2f",
                (int)c1, (int)c2_same, (int)c2_outdir, (int)c3, dist,
                node_ptr->position.z, goal_node_ptr->position.z);
            logged = true;
        }
        if (c1 && c2 && c3) {
            return true;
                              ...
                              bool& _is_free_nav) 
{
    ...
    RCLCPP_WARN(nh_->get_logger(), "PathToGoal: parent=%s free_parent=%s free_nav=%d connect_nodes=%zu gscore=%.1f",
        goal_ptr->parent ? "SET" : "NULL", goal_ptr->free_parent ? "SET" : "NULL",
        (int)is_free_nav_goal_, goal_ptr->connect_nodes.size(), goal_ptr->gscore);
