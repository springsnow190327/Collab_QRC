...
void FARMaster::MainLoopCallBack() {
  ...
  if (FARUtil::IsMultiLayer) {
    std::vector<PointCloudPtr> obs_layer_clouds;
    std::vector<float> obs_layer_centers;
    map_handler_.GetSurroundObsCloudLayers(obs_layer_clouds, obs_layer_centers);
    std::vector<std::vector<PointStack>> layered_contours;
    contour_detector_.BuildTerrainImgAndExtractContourLayered(
        odom_node_ptr_, obs_layer_clouds, obs_layer_centers, layered_contours);
    contour_graph_.UpdateContourGraph(odom_node_ptr_, layered_contours, obs_layer_centers);
    // Retain the most recent layer's contours for any single-layer consumers
    // (e.g. viewpoint extension) that still peek at realworld_contour_.
    realworld_contour_.clear();
    if (!layered_contours.empty()) {
      realworld_contour_ = layered_contours.back();
    }
  } else {
...
void FARMaster::LoadROSParams() {
  ...
  nh_->declare_parameter<float>(planner_prefix + "/layer_transition_cost_mult", 2.0f);
  ...
  nh_->get_parameter(planner_prefix + "/layer_transition_cost_mult", gp_params_.layer_transition_cost_mult);
  ...
  nh_->declare_parameter<float>(contour_prefix + "/oversize_range_ratio", 3.0f);
  nh_->get_parameter(contour_prefix + "/oversize_range_ratio", cg_params_.kOversizeRangeRatio);
...
std::vector<std::pair<CTNodePtr, CTNodePtr>> ContourGraph::layer_transitions_;
...
std::unique_ptr<grid_ns::Grid<std::vector<float>>> MapHandler::terrain_multilayer_heights_;
