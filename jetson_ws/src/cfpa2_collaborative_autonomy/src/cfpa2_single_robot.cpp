...
: CFPA2Coordinator(nh, pnh, single_robot_opts())
{
  init_single_robot();
}

CFPA2SingleRobotNode::CFPA2SingleRobotNode()
: CFPA2Coordinator(single_robot_opts())
{
  init_single_robot();
}

void CFPA2SingleRobotNode::init_single_robot()
{
  ...
  subs_.push_back(nh_.subscribe<geometry_msgs::PoseArray>(
      blocked_topic, 10,
      boost::function<void(const geometry_msgs::PoseArray::ConstPtr &)>(
          [this](const geometry_msgs::PoseArray::ConstPtr & msg) {
  ...
  if (get_parameter("ramp_ascent_enabled").as_bool()) {
    ...
    subs_.push_back(nh_.subscribe<geometry_msgs::PointStamped>(
        topic, 10,
