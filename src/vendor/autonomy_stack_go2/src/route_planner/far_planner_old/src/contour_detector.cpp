...
#include "far_planner_old/contour_detector.h"
...
void ContourDetector::BuildTerrainImgAndExtractContour(const NavNodePtr& odom_node_ptr,
    ...
    // ── DIAG STAGE 3: contour_detector output ────────────
    {
      static int _diag_ctr3 = 0;
      if (++_diag_ctr3 % 10 == 1) {
        std::cout << "[DIAG3] extracted CV_contours=" << refined_contours_.size()
                  << " realworld_contours=" << realworl_contour.size() << std::endl;
      }
    }
    // ────────────────────────────────────────────────────────
}
