...
void GraphExtractor::LoadParmas() {
    std::string default_folder_path = "data/";
    try {
        default_folder_path = ament_index_cpp::get_package_share_directory("far_planner") + "/data/";
    } catch (const std::exception&) {
        // Keep relative fallback if package discovery is unavailable.
    }

    // Declare parameters if they haven't been declared yet
    nh_->declare_parameter("world_frame", "map");
    nh_->declare_parameter("folder_path", default_folder_path);
