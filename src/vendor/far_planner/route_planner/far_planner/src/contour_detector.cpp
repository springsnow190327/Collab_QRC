...
void ContourDetector::BuildTerrainImgAndExtractContourLayered(
    const NavNodePtr& odom_node_ptr,
    const std::vector<PointCloudPtr>& layer_clouds,
    const std::vector<float>& layer_z_centers,
    std::vector<std::vector<PointStack>>& layered_contours) {
    layered_contours.clear();
    const std::size_t n_layers = layer_clouds.size();
    layered_contours.resize(n_layers);
    if (n_layers == 0) return;
    this->UpdateOdom(odom_node_ptr);
    for (std::size_t li = 0; li < n_layers; li++) {
        (void)layer_z_centers;  // z tag is consumed by the caller; detector is 2D
        this->ResetImgMat(img_mat_);
        if (layer_clouds[li] && !layer_clouds[li]->empty()) {
            this->UpdateImgMatWithCloud(layer_clouds[li], img_mat_);
        }
        this->ExtractContourFromImg(img_mat_, refined_contours_, layered_contours[li]);
        // After the last layer, leave img_mat_ containing that layer so
        // downstream viz (ShowCornerImage) behaves as it did before.
    }
}
